...
{
    /// <summary>
    /// MySql 数据库的 Sql 创建工厂类。
    /// </summary>
    public class SqlFactory : ISqlFactory
    {
        private const string _CASE_SENSITIVE_MARK = "binary";
                ...
                switch (field.OperationType)
                {
                    case OperationType.Add:
                        if (field.Left.DataType == _StringType || field.Right.DataType == _StringType)
                            return new ExpressionInfo($"concat_ws('',{leftSql},{rightSql})", OperationType.Call);
                        else
                            return new ExpressionInfo($"{leftSql} + {rightSql}", field.OperationType);
                    case OperationType.And:
                        return new ExpressionInfo($"{leftSql} & {rightSql}", field.OperationType);
                    case OperationType.Divide:
                        if ((field.Left.DataType == _IntType
                            || field.Left.DataType == _UIntType
                            || field.Left.DataType == _ShortType
                            || field.Left.DataType == _UShortType
                            || field.Left.DataType == _ByteType
                            || field.Left.DataType == _SByteType
                            || field.Left.DataType == _LongType
                            || field.Left.DataType == _ULongType) && (field.Right.DataType == _IntType
                            || field.Right.DataType == _UIntType
                            || field.Right.DataType == _ShortType
                            || field.Right.DataType == _UShortType
                            || field.Right.DataType == _ByteType
                            || field.Right.DataType == _SByteType
                            || field.Right.DataType == _LongType
                            || field.Right.DataType == _ULongType))
                        {
                            return new ExpressionInfo($"floor({leftSql} / {rightSql})", OperationType.Call);
                        }
                        else
                        {
                            return new ExpressionInfo($"{leftSql} / {rightSql}", field.OperationType);
            ...
            if (field.OperationType == OperationType.In || field.OperationType == OperationType.NotIn)
            {
                ...
                if (field.Right.Type == FieldType.Constant)
                {
                    ...
                    if (field.Left.DataType == _StringType && field.DataContext.IsCaseSensitive)
                        leftSql = $"{_CASE_SENSITIVE_MARK} {leftSql}";
            ...
            else if (field.OperationType == OperationType.Equal || field.OperationType == OperationType.NotEqual)
            {
                ...
                else
                {
                    ...
                    if (field.DataContext.IsCaseSensitive && field.Left.DataType == _StringType && field.Right.DataType == _StringType)
                        return new ExpressionInfo($"{_CASE_SENSITIVE_MARK} {leftSql} {(field.OperationType == OperationType.Equal ? "=" : "!=")} {rightSql}", field.OperationType);
                    else
                ...
                switch (field.OperationType)
                {
                    ...
                    case OperationType.NotLike:
                        if (field.DataContext.IsCaseSensitive)
                            return new ExpressionInfo($"{_CASE_SENSITIVE_MARK} {leftSql} {(field.OperationType == OperationType.Like ? "like" : "not like")} {rightSql}", field.OperationType);
                        else
        ...
        public ExpressionInfo CreateConditionalBasicSql(IParameterContext parameterContext, ConditionalField field)
        {
            ...
            return new ExpressionInfo($"if({testSql}, {ifTrueSql}, {ifFalseSql})", OperationType.Call);
        }
        ...
        public ExpressionInfo CreateConstantBasicSql(IParameterContext parameterContext, ConstantField field)
        {
            ...
            if (Helper.CheckIsBasicType(field.DataType))
                ...
                throw new Exception($"MySql 子查询不支持多行数据返回，若要使用子查询，可在 {nameof(Queryable)} 对象最后调用一次 First() 函数或 Last() 函数将其限定只返回一行数据");
                ...
                throw new Exception("MySql 只支持基础数据类型的常量作为参数传入");
        }
            ...
            if (field.ObjectField != null && field.ObjectField is BasicField basicField)
            {
                ...
                else if (field.ObjectField.DataType == _DateTimeType && field.MemberInfo.MemberType == MemberTypes.Property)
                {
                        ...
                        || field.MemberInfo.Name == "Millisecond")
                    {
                        ...
                        if (field.MemberInfo.Name == "Millisecond")
                            return new ExpressionInfo($"floor(microsecond({objectSql}) / 1000)", OperationType.Call);
                        else
                            return new ExpressionInfo($"{field.MemberInfo.Name.ToLower()}({objectSql})", OperationType.Call);
                    }
        ...
        public ExpressionInfo CreateMethodBasicSql(IParameterContext parameterContext, MethodField field)
        {
                ...
                if (field.ObjectField is BasicField basicField)
                {
                    ...
                    if (field.ObjectField.DataType == _StringType)
                    {
                        ...
                        if (field.MethodInfo.Name == "IndexOf" && field.Parameters != null && field.Parameters.Count == 1 && field.Parameters[0] is BasicField parameter)
                        {
                            ...
                            if (field.DataContext.IsCaseSensitive)
                                objectSql = $"{_CASE_SENSITIVE_MARK} {objectSql}";

                            //MySql 中的 instr 是从 1 开始，结果得手动减去 1，startIndex 得手动加上 1
                        ...
                        else if (field.MethodInfo.Name == "Substring" && field.Parameters != null && (field.Parameters.Count == 1 || field.Parameters.Count == 2))
                        {
                            ...
                            { 
                                return new ExpressionInfo($"substr({stringSql}, {startSql} + 1)", OperationType.Call);
                            }
                            else
                            {
                        ...
                        else if ((field.MethodInfo.Name == "Insert" || field.MethodInfo.Name == "Replace") && field.Parameters != null && field.Parameters.Count == 2 && field.Parameters[0] is BasicField && field.Parameters[1] is BasicField)
                        {
                            ...
                            if (field.MethodInfo.Name == "Replace")
                                ...
                                return new ExpressionInfo($"concat_ws('',substr({objectSql}, 1, {param1Sql}),{param2Sql},substr({objectSql},{(Helper.CheckIsPriority(OperationType.Add, param1.Type, false) ? $"({param1.SQL})" : param1.SQL)} + 1))", OperationType.Add);
                        }
                    }
                    //若调用实例为 DateTime 类型
                    else if (field.ObjectField.DataType == _DateTimeType)
                    {
                        ...
                        if (field.MethodInfo.Name == "ToString")
                        {
                            ...
                            if (field.Parameters != null && field.Parameters.Count > 0)
                            {
                                ...
                                {
                                    throw new Exception("MySql 不支持多参数的日期格式化函数");
                                }
                            }
                            ...
                            return new ExpressionInfo($"date_format({dateTimeSql}, {format})", OperationType.Call);
                        }
                    }
                    //无参 ToString 方法
                    else if (field.MethodInfo.Name == "ToString" && (field.Parameters == null || field.Parameters.Count < 1))
                    {
                        ...
                        return new ExpressionInfo($"convert({objectSql}, char)", OperationType.Call);
                    }
                }
            }
            ...
            else if (field.MethodInfo.DeclaringType.FullName == "System.Convert")
            {
                ...
                if (field.MethodInfo.Name.StartsWith("To") && field.Parameters != null && field.Parameters.Count == 1 && field.Parameters[0] is BasicField basicField)
                {
                    ...
                    switch (field.MethodInfo.Name)
                    {
                        ...
                        case "ToBoolean":
                        case "ToByte":
                            return new ExpressionInfo($"floor(convert({parameterSql}, decimal(21, 6)))", OperationType.Call);
                        case "ToSingle":
                        case "ToDouble":
                        case "ToDecimal":
                            return new ExpressionInfo($"convert({parameterSql}, decimal(21, 6))", OperationType.Call);
                        case "ToDateTime":
                            return new ExpressionInfo($"convert({parameterSql}, datetime)", OperationType.Call);
                    }
                }
            }
            //如果是 Math 数学函数类
            ...
            else if (field.MethodInfo.ReflectedType == _DBFunType)
            {
                ...
                else if (field.Parameters != null && field.Parameters.Count == 1 && (field.MethodInfo.Name == _DBFunMaxMethodName || field.MethodInfo.Name == _DBFunSTDevMethodName || field.MethodInfo.Name == _DBFunSTDevPMethodName || field.MethodInfo.Name == _DBFunMinMethodName || field.MethodInfo.Name == _DBFunSumMethodName || field.MethodInfo.Name == _DBFunAvgMethodName || field.MethodInfo.Name == _DBFunVarMethodName || field.MethodInfo.Name == _DBFunVarPMethodName) && field.Parameters[0] is BasicField basicField)
                {
                    ...
                    if (field.MethodInfo.Name == _DBFunMaxMethodName)
                        ...
                        throw new Exception($"MySql 不支持 DBFun.{field.MethodInfo.Name} 函数");
                ...
                {
                    return new ExpressionInfo("now()", OperationType.Call);
                }
                //DBFun.NewInt 函数
                else if ((field.Parameters == null || field.Parameters.Count < 1 || field.Parameters.Count == 2) && field.MethodInfo.Name == _DBFunNewIntMethodName)
                {
                    if (field.Parameters == null || field.Parameters.Count < 1)
                    {
                        return new ExpressionInfo("floor((rand() * 4294967295) - 2147483648)", OperationType.Call);
                    }
                    else
                    {
                        ...
                        return new ExpressionInfo($"{minSql} + floor(rand() * {maxSql})", OperationType.Add);
                    }
                }
                //DBFun.NewLong 函数
                    ...
                    || field.MethodInfo.Name == _DBFunDiffMillisecondMethodName))
                {
                    ...
                    if (field.MethodInfo.Name == _DBFunDiffMillisecondMethodName)
                    {
                        return new ExpressionInfo($"floor(timestampDiff(microsecond, {objectSql}, {compareSql}) / 1000)", OperationType.Call);
                    }
                    else
                    {
                        string type;
                        ...
                        return new ExpressionInfo($"timestampDiff({type}, {objectSql}, {compareSql})", OperationType.Call);
                    }
                }
                //DNFun 的各种日期添加函数
                    ...
                    || field.MethodInfo.Name == _DBFunAddMillisecondMethodName))
                {
                    ...
                    {
                        string valueSql = Helper.CheckIsPriority(OperationType.Multiply, value.Type, false) ? $"({value.SQL})" : value.SQL;

                        return new ExpressionInfo($"date_add({objectSql}, interval {valueSql} * 1000 microsecond)", OperationType.Call);
                    }
                    else
                    {
                        string type;
                        ...
                        return new ExpressionInfo($"date_add({objectSql}, interval {valueSql} {type})", OperationType.Call);
                    }
                }
            }
            //各种 Parse 方法
            else if (field.MethodInfo.Name == "Parse" && field.Parameters != null && field.Parameters.Count == 1 && field.Parameters[0].DataType == _StringType && field.Parameters[0] is BasicField parameterField)
            {
                ...
                else if (field.DataType == _BoolType && field.MethodInfo.ReflectedType == _BoolType)
                {
                    ...
                    return new ExpressionInfo($"lower(trim({parameterSql})) != 'false' and length(trim({parameterSql})) > 0", OperationType.Call);
                }
                //Guid.Parse 方法
        ...
        public ExpressionInfo CreateMethodBoolSql(IParameterContext parameterContext, MethodField field)
        {
            ...
            if (field.ObjectField != null && field.MethodInfo.ReflectedType == _StringType && field.ObjectField is BasicField basicField && (field.MethodInfo.Name == "StartsWith" || field.MethodInfo.Name == "EndsWith" || field.MethodInfo.Name == "Contains") && field.Parameters != null && field.Parameters.Count == 1 && field.Parameters[0] is BasicField parameter)
            {
                ...
                {
                    if (field.DataContext.IsCaseSensitive)
                        return new ExpressionInfo($"instr({_CASE_SENSITIVE_MARK} {objectSql}, {searchString}) == length({objectSql}) - length({searchString}) + 1", OperationType.Equal);
                    else
                ...
                else
                {
                    ...
                    if (field.DataContext.IsCaseSensitive)
                        return new ExpressionInfo($"instr({_CASE_SENSITIVE_MARK} {objectSql}, {searchString}) {checkString} 1", field.MethodInfo.Name == "StartsWith" ? OperationType.Equal : OperationType.GreaterThanOrEqual);
                    else
        ...
        public ExpressionInfo CreateUnaryBasicSql(IParameterContext parameterContext, UnaryField field)
        {
            ...
            else if (field.OperationType == OperationType.Convert)
            {
                ...
                {
                    return new ExpressionInfo($"floor(convert({operandSql}, decimal(21, 6)))", OperationType.Call);
                }
                else if (field.DataType == _FloatType
                    || field.DataType == _DoubleType
                    || field.DataType == _DecimalType)
                {
                    return new ExpressionInfo($"convert({operandSql}, decimal(21, 6))", OperationType.Call);
                }
                else if (field.DataType == _DateTimeType)
                {
                    return new ExpressionInfo($"convert({operandSql}, datetime)", OperationType.Call);
                }
                else if (field.DataType == _StringType)
                {
                    return new ExpressionInfo($"convert({operandSql}, char)", OperationType.Call);
                }
                else
                {
                    return operand;
                ...
                if (field.Operand.Type == FieldType.Method)
                {
                    ...
                    if (operandMethodField.MethodInfo.ReflectedType == _StringType && operandMethodField.Parameters != null && operandMethodField.Parameters.Count == 1 && operandMethodField.Parameters[0].DataType == _StringType && operandMethodField.Parameters[0] is BasicField parameter)
                    {
                        ...
                        else if (operandMethodField.ObjectField != null && operandMethodField.ObjectField is BasicField basicField && (operandMethodField.MethodInfo.Name == "StartsWith" || operandMethodField.MethodInfo.Name == "Contains"))
                        {
                            ...
                            if (field.DataContext.IsCaseSensitive)
                                return new ExpressionInfo($"instr({_CASE_SENSITIVE_MARK} {objectSql}, {searchString}) {checkString} 1", operandMethodField.MethodInfo.Name == "StartsWith" ? OperationType.NotEqual : OperationType.LessThan);
                            else
                                return new ExpressionInfo($"instr({_CASE_SENSITIVE_MARK} {objectSql}, {searchString}) {checkString} 1", operandMethodField.MethodInfo.Name == "StartsWith" ? OperationType.NotEqual : OperationType.LessThan);
                        }
                        ...
                        else if (operandMethodField.ObjectField != null && operandMethodField.ObjectField is BasicField stringField && operandMethodField.MethodInfo.Name == "EndsWith")
                        {
                            ...
                            if (field.DataContext.IsCaseSensitive)
                                return new ExpressionInfo($"instr({_CASE_SENSITIVE_MARK} {objectSql}, {searchString}) != length({objectSql}) - length({searchString}) + 1", OperationType.NotEqual);
                            else
        ...
        public string GenerateDeleteSql(IParameterContext parameterContext, TableDataSource[] deletingDataSources, DataSource.DataSource fromDataSource, BasicField where)
        {
            ...
            else
            {
                ...
                if (where != null)
                    deleteSql.Append($" where {where.GetBoolExpression(parameterContext).SQL}");
        ...
        public string GenerateUpdateSql(IParameterContext parameterContext, ReadOnlyList<UpdateGroup> updateGroups, DataSource.DataSource fromDataSource, BasicField where)
        {
            ...
            {
                foreach (var item in group.UpdateFields)
                {
                    if (setSql.Length > 0)
                        setSql.Append(",");
            ...
            if (where == null)
                result = $"update {GenerateFromSql(parameterContext, fromDataSource, false)} set {setSql}";
            else
                result = $"update {GenerateFromSql(parameterContext, fromDataSource, false)} set {setSql} where {where.GetBoolExpression(parameterContext).SQL}";
        ...
        {
            return $"select count(1) from information_schema.schemata where schema_name = {parameterContext.Add(databaseInfo.Name)}";
        }
        ...
        internal string GenerateCreateDatabaseSql(IParameterContext parameterContext, DatabaseInfo databaseInfo)
        {
            ...
            createSql = new StringBuilder($"create database {databaseInfo.Name}");

            if (!string.IsNullOrWhiteSpace(extraInfo?.CharActer))
            {
                createSql.Append($" character set '{extraInfo.CharActer}'");

                if (!string.IsNullOrWhiteSpace(extraInfo.Collate))
                    createSql.Append($" collate '{extraInfo.Collate}'");
            }
        ...
        /// <param name="databaseName">校验表所在的数据库名称。</param>
        /// <returns>生成好的校验 Sql 语句。</returns>
        internal string GenerateExistsTableSql(IParameterContext parameterContext, string tableName, string databaseName)
        {
            return $"select count(1) from information_schema.tables where table_name = {parameterContext.Add(tableName)} and table_schema = {parameterContext.Add(databaseName)}";
        }
        ...
        /// <param name="databaseName">校验视图所在的数据库名称。</param>
        /// <returns>生成好的校验 Sql 语句。</returns>
        internal string GenerateExistsViewSql(IParameterContext parameterContext, string viewName, string databaseName)
        {
            return $"select count(1) from information_schema.views where table_name = {parameterContext.Add(viewName)} and table_schema = {parameterContext.Add(databaseName)}";
        }
        ...
        internal string GenerateCreateTableSql(IParameterContext parameterContext, TableDataSource tableDataSource)
        {
            ...
            foreach (var item in ((ObjectField)tableDataSource.RootField).Members)
            {
                ...
                if (field.DefaultValue != null)
                {
                    if (field.DataType == _IntType
                        ...
                        || field.DataType == _DoubleType
                        || field.DataType == _DecimalType
                        || field.DataType == _DateTimeType && new Regex(@"^\s*now\s*\(\s*\)\s*$", RegexOptions.IgnoreCase).IsMatch(field.DefaultValue.ToString()))
                    {
                ...
                if (!string.IsNullOrWhiteSpace(field.Explain))
                    fields.Append($" comment '{field.Explain}'");

                if (field.IsUnique)
                    indices.Append($",{Environment.NewLine} unique index `{tableDataSource.Name.Trim('`')}_{field.FieldName.Trim('`')}_Unique`({field.FieldName})");

                if (!string.IsNullOrWhiteSpace(field.CheckConstraint))
                    checkConstraints.Append($",{Environment.NewLine} constraint `{tableDataSource.Name.Trim('`')}_{field.FieldName.Trim('`')}_CheckConstraint` check ({field.CheckConstraint})");
            }
            ...
            if (tableDataSource.Indices != null && tableDataSource.Indices.Count > 0)
            {
                foreach (var item in tableDataSource.Indices)
                {
                    if (item.Type == IndexType.Unique)
                        indices.Append($",{Environment.NewLine} unique index ");
                    else if (item.Type == IndexType.FullText)
                        indices.Append($",{Environment.NewLine} fullText index ");
                    else
                        indices.Append($",{Environment.NewLine} index ");
            ...
            if (!string.IsNullOrWhiteSpace(primaryKeyField))
                createSql.Append($",{Environment.NewLine} primary key ({primaryKeyField})");

            if (indices.Length > 0)
                createSql.Append(indices);

            if (checkConstraints.Length > 0)
                createSql.Append(checkConstraints);
        ...
        /// </summary>
        /// <param name="parameterContext">生成 Sql 时用于参数化操作的上下文对象。</param>
        /// <param name="databaseName">所在数据库名称。</param>
        /// <returns>生成好的查询 Sql 语句。</returns>
        internal string GenerateSelectAllTableNameSql(IParameterContext parameterContext, string databaseName)
        {
            return $"select count(1) from information_schema.tables where table_schema = {parameterContext.Add(databaseName)}";
        }
        ...
        /// <param name="databaseName">所在数据库名称。</param>
        /// <returns>生成好的查询 Sql 语句。</returns>
        internal string GenerateSelectAllViewNameSql(IParameterContext parameterContext, string databaseName)
        {
            return $"select count(1) from information_schema.views where table_schema = {parameterContext.Add(databaseName)}";
        }
            ...
            else
            {
                ...
                if (belongDataSource.Limit != null && belongDataSource.Limit.HasValue)
                {
                    if (belongDataSource.Limit.Value.Start == 0)
                        other = $" limit {belongDataSource.Limit.Value.Count}";
                    else
                        other = $" limit {belongDataSource.Limit.Value.Count} offset {belongDataSource.Limit.Value.Start}";
                }

                return $"select {distinct}{sqlFields} from {sqlFrom}{sqlWhere}{groupBy}{orderBy}{other}";
            }
        }
        ...
        private string GenerateUnionSql(IParameterContext parameterContext, UnionDataSource unionDataSource)
        {
            ...
            switch (unionDataSource.UnionType)
            {
                ...
                case UnionType.Minus:
                    ...
                    break;
                default:
                    throw new Exception($"MySql 不支持 {unionDataSource.UnionType} 的方式合并数据源");
            }
